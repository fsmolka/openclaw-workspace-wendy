...
function qualityWord(value, good, watch, highIsGood = true) {
  if (highIsGood) {
    if (value >= good) return "Good";
    if (value >= watch) return "Close";
    return "Low";
  }
  if (value <= good) return "Good";
...
function renderQuickRead(ctx) {
  const targetProtein = DATA.goal.protein_target_g || 150;
  const targetDeficit = DATA.goal.deficit_target_kcal || 500;
  const proteinStatus = qualityWord(ctx.avgProtein, targetProtein, targetProtein - 20);
  const fiberStatus = qualityWord(ctx.avgFiber, 28, 22);
  const fatStatus = qualityWord(ctx.avgFat, 85, 105, false);
  const deficitStatus = ctx.avgDeficit > targetDeficit + 350
    ? "Aggressive"
    : ctx.avgDeficit < targetDeficit - 250
      ? "Light"
      : "Controlled";
  const today = ctx.latest;
  const todayPartial = (today.quality_flags || []).includes("partial_day");
  const lowSleep = ctx.avgSleep < 6.7;
  const volatile = ctx.intakeRange > 800;

  const headline = lowSleep
    ? "Nutrition is carrying the week; recovery needs respect."
    : deficitStatus === "Aggressive"
      ? "Progress is real, but do not chase a bigger cut."
      : "The plan is working when the day stays repeatable.";

  const summaryParts = [
    proteinStatus === "Good" ? "protein is strong enough to protect lean mass" : "protein needs to be brought back up",
    deficitStatus === "Controlled" ? "the calorie pace is useful" : `the calorie pace is ${deficitStatus.toLowerCase()}`,
    fiberStatus === "Good" ? "fiber looks supportive" : "fiber is a satiety lever",
    lowSleep ? "sleep is the main weak link" : "recovery is acceptable"
  ];

  setText("quickHeadline", headline);
  setText("quickSummary", summaryParts.join("; ") + ".");

  setText("quickProtein", `${proteinStatus}: ${fmt.format(ctx.avgProtein)}g/day`);
  setText(
    "quickProteinDetail",
    proteinStatus === "Good"
      ? "This is the strongest nutrition signal. Keep it stable before changing calories."
      : `Add about ${fmt.format(Math.max(0, targetProtein - ctx.avgProtein))}g/day before cutting harder.`
  );

  setText("quickMacros", `${fatStatus} fat, ${fiberStatus.toLowerCase()} fiber`);
  setText(
    "quickMacrosDetail",
    `Average fat is ${fmt.format(ctx.avgFat)}g and fiber is ${fmt.format(ctx.avgFiber)}g. Fat quality needs food-level detail; this read judges amount and calorie pressure.`
  );

  setText("quickCalories", `${deficitStatus}: ${fmt.format(ctx.avgDeficit)} kcal/day`);
  setText(
    "quickCaloriesDetail",
    volatile
      ? "The weekly deficit works, but intake swings are big. Make the next few days boring and repeatable."
      : "This is a useful cutting pace if hunger and training stay controlled."
  );

  setText("quickMove", todayPartial ? "Still building" : `${fmt.format(today.steps || 0)} steps`);
  setText(
    "quickMoveDetail",
    todayPartial
      ? `Today is partial. So far: ${fmt.format(today.protein_g || 0)}g protein, ${fmt.format(today.steps || 0)} steps, ${fmt1.format(today.sleep_hr || 0)}h sleep.`
      : `Latest complete day: ${fmt.format(today.steps || 0)} steps and ${fmt1.format(today.sleep_hr || 0)}h sleep.`
  );
}
